...
class Workermanjnd28Controller extends Server {
	...
	public function onWorkerStart(){
		...
		if($nexttime-time()>C('jnd28_stop_time') && $nexttime-time()<165){
			F('jnd28_state',1);
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			if($next_time-time()>C('jnd28_stop_time') && $next_time-time()<160){
			...
			if($next_time-time()== C('jnd28_stop_time')){
				$new_message = array(
					...
					'content'=>$bj28data['next']['periodNumber'].'期距离封盘还有30秒,请各位玩家抓紧下注',
			...
			if($next_time-time()==C('jnd28_stop_time')){
				...
				$new_message = array(
					...
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$jnd28data['next']['periodNumber'].'关闭，请耐心等待开奖',
					'time' => date('Y-m-d H:i:s')
				);
				foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode($new_message));
			...
			if($next_time-time()<165 && $next_time-time()>100 && F('is_send')==0){
				//结算
				//开奖结果
								...
								if($num1>0){
									if ($current_number['tema'] == '13' || $current_number['tema'] == '14') {
										if(C('jnd28'.$fangjian.'_1314_open') == '1'){
											...
											if(intval($user_points['sum_del']) < intval(C('jnd28'.$fangjian.'_dxds_1314zz'))){
												$points1 = $num1*$starts1*C('jnd28'.$fangjian.'_dxds_md1');
											}else if(intval($user_points['sum_del']) >= intval(C('jnd28'.$fangjian.'_dxds_1314zz')) && intval($user_points['sum_del']) < intval(C('jnd28'.$fangjian.'_dxds_1314zz2'))){
												$points1 = $num1*$starts1*C('jnd28'.$fangjian.'_dxds_md2');
											}else if(intval($user_points['sum_del']) >= intval(C('jnd28'.$fangjian.'_dxds_1314zz2'))){
												$points1 = $num1*$starts1*C('jnd28'.$fangjian.'_dxds_md3');
											}
										}else{
											$points1 = $num1*$starts1*C('jnd28'.$fangjian.'_dxds_md');
										}
									} else {
										$points1 = $num1*$starts1*C('jnd28'.$fangjian.'_dxds');
									}
								...
								if($num2>0){
									if ($current_number['tema'] == '13' || $current_number['tema'] == '14') {
										...
										if ($start2 == '大单' || $start2 == '小双') {
											$points2 = $num2*$starts2*C('jnd28'.$fangjian.'_zuhe_1');
										} else {
											$points2 = $num2*$starts2*C('jnd28'.$fangjian.'_zuhe_2');
										}
									}
								...
								if($num4 > 0 ){
									if ($start4 == '庄' || $start4 == '闲') {
										if ($current_number['zx'] == '和') {
											...
											$points4 = $num4*$starts4*C('jnd28'.$fangjian.'_zx_1');
										}
									} else {
							...
							case 5:
								...
								if($num5>0){
									if ($start5 == '豹子') {
										...
										$points5 = $num5*$starts5*C('jnd28'.$fangjian.'_bds_2');
									} else if($start5 == '对子') {
										$points5 = $num5*$starts5*C('jnd28'.$fangjian.'_bds_3');
									} else if($start5 == '半顺') {
										$points5 = $num5*$starts5*C('jnd28'.$fangjian.'_bds_4');
									} else if($start5 == '杂六') {
										$points5 = $num5*$starts5*C('jnd28'.$fangjian.'_bds_5');
									}
							...
							case 6:
								...
								if($num6>0){
									if ($start6[0] == '0' || $start6[0] == '27') {
										...
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_1');
									} else if($start6[0] == '2' || $start6[0] == '25') {
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_2');
									} else if($start6[0] == '3' || $start6[0] == '24') {
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_3');
									} else if($start6[0] == '4' || $start6[0] == '23') {
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_4');
									} else if($start6[0] == '5' || $start6[0] == '22') {
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_5');
									} else if($start6[0] == '6' || $start6[0] == '21') {
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_6');
									} else if($start6[0] == '7' || $start6[0] == '20') {
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_7');
									} else if($start6[0] == '8' || $start6[0] == '19') {
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_8');
									} else if($start6[0] == '9' || $start6[0] == '18') {
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_9');
									} else if($start6[0] == '10' || $start6[0] == '17') {
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_10');
									} else if($start6[0] == '11' || $start6[0] == '16') {
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_11');
									} else if($start6[0] == '12' || $start6[0] == '15') {
										$points6 = $num6*$start6[1]*C('jnd28'.$fangjian.'_tema_12');
				...
				$new_message = array(
					...
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$jnd28data['next']['periodNumber'].'开放，祝各位中大奖',
					'time' => date('Y-m-d H:i:s')
				);
		...
		\Workerman\Lib\Timer::add(5, function(){
			...
			if(F('jnd28PeriodNumber')!=$data['current']['periodNumber']){
				...
				if(!$res){
					...
					if($res1){
						F('jnd28PeriodNumber',$data['current']['periodNumber']);
						F('jnd28data',$data);
						F('is_send',0);
					}
				}
		...
		switch($message_data['type']){
			...
			case 'say' :
				...
				if($jnd28_state == 0){
					...
					/*检测格式和金额*/
					$res = check_format_bj28($message_data['content'],C('jnd28qi_min_point'),C('jnd28qi_max_point'),C('jnd28_xz_open'),C('jnd28_xz_max'));
					if($res['error']==0 || $res['error']==2){
						...
						if ($res['error'] == 0) {
							$content_msg =  '「'.$message_data['content'].'」'.'单笔点数最低'.C('jnd28qi_min_point').',竞猜失败';
						} else {
						...
						if($jifen['points']<$res['points']){
							...
							if ((intval($user_points['sum_del'])+$res['points']) > C('jnd28qi_max_point')) {
								$points_tips = array(
									...
									'head_img_url'=>'/Public/main/img/kefu.jpg',
									'from_client_name' => 'GM管理员',
									'content' => '「'.$message_data['content'].'」'.'每期最高点数'.$res['jnd28qi_max_points'].',您已超过，竞猜失败',
									'time' => date('Y-m-d H:i:s')
								);
								$connection -> send(json_encode($points_tips));
								$points_tips['type'] = 'error';
							...
							$map['time'] = time();
							$map['number'] = $jnd28data['next']['periodNumber'];
							...
							$map['fangjian'] = $message_data['fangjian'];
							$map['game'] = F('game');
							$map['t_id'] = $user['t_id'];
