...
class Workermansb28Controller extends Server {

	protected $socket = 'websocket://0.0.0.0:14725';
	...
	public function onWorkerStart(){
		...
		if($nexttime-time()>C('bj28_stop_time') && $nexttime-time()<285 && time()>$beginToday && time()<$endToday){
			F('onebj28_state',1);
			setconfig('onebj28_state',1);
		}else{
			F('onebj28_state',0);
			setconfig('onebj28_state',0);
		}
		if(!F('onebj28data')){
			F('onebj28data',$bj28data);
		}
		if(!F('one_is_send')){
			F('one_is_send',1);
		}
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			foreach ($this->worker->connections as $conn) {
						$conn->send(json_encode(['now'=>date('H:i:s'),'next'=>date('H:i:s',$next_time),'cha'=>$next_time-time()]));
					}
			...
			if((($next_time-time()<285 && $next_time-time()>100) || (time() > strtotime("23:52:00") && time() < strtotime("23:55:00"))) && F('one_is_send')==0){
				//结算
				//开奖结果
		...
		\Workerman\Lib\Timer::add(5, function(){
			...
			foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode(['now'=>F('onebj28PeriodNumber'),'send'=>$data['current']['periodNumber']]));
				}
			if(F('onebj28PeriodNumber')!=$data['current']['periodNumber']){
				...
				$res = M('number')->where("game = 'bj28' and periodnumber = {$data['current']['periodNumber']} and awardtime > '{$today_time}'")->find();
							foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode(['data'=>$res]));
				}
				// var_dump($res);
				if(!$res){
					...
					$res1 = M('number')->add($map);
									foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode(['insertdata'=>$res1]));
				}
					if($res1){
						F('onebj28PeriodNumber',$data['current']['periodNumber']);
						F('onebj28data',$data);
						 F('one_is_send',0);
					}
				}else{
		...
		switch($message_data['type']){
			...
			case 'say' :
				...
				if($onebj28_state == 0){
					...
					$res = check_format_bj28($message_data['content'],$limit[1][0],C('bj28qi_max_point'),C('bj28_xz_open'),C('bj28_xz_max'));
					if($res['error']==0 || $res['error']==2){
						...
						if ($res['error'] == 0) {
							$content_msg =  '「'.$message_data['content'].'」'.'单笔点数最低'.$limit[1][0].',竞猜失败';
						} else {
