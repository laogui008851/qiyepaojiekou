...
class Workermansb228Controller extends Server {

	protected $socket = 'websocket://0.0.0.0:25836';
	...
	public function onWorkerStart(){
		...
		if($nexttime-time()>C('bj28_stop_time') && $nexttime-time()<285 && time()>$beginToday && time()<$endToday){
			F('twobj28_state',1);
			setconfig('twobj28_state',1);
			echo 'start';
		}else{
			F('twobj28_state',0);
			setconfig('twobj28_state',0);

			echo 'stop';
		}
		if(!F('twobj28data')){
			F('twobj28data',$bj28data);
			echo 'nohave';
		}
		if(!F('two_is_send')){
			F('two_is_send',1);
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			if($next_time-time()>C('bj28_stop_time') && $next_time-time()<280 && time()>$beginToday && time()<$endToday ){
				...
				setconfig('twobj28_state',1);
					foreach ($this->worker->connections as $conn) {
						$conn->send(json_encode(['status'=>1]));
					}
			}else{
				F('twobj28_state',0);
				setconfig('twobj28_state',0);
				foreach ($this->worker->connections as $conn) {
						$conn->send(json_encode(['status'=>0]));
					}
			}
			...
			if((($next_time-time()<285 && $next_time-time()>100) || (time() > strtotime("23:52:00") && time() < strtotime("23:55:00"))) && F('two_is_send')==0){
				//结算
				//开奖结果
		...
		\Workerman\Lib\Timer::add(5, function(){
			...
			$data = json_decode(bj28_format($caiji),true);
						foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode(['now'=>F('twobj28PeriodNumber'),'send'=>$data['current']['periodNumber']]));
				}
			if(F('twobj28PeriodNumber')!=$data['current']['periodNumber']){
				...
				if(!$res){
					...
					if($res1){
						F('twobj28PeriodNumber',$data['current']['periodNumber']);
						F('twobj28data',$data);
						F('two_is_send',0);
					}
				}else{
		...
		switch($message_data['type']){
			...
			case 'say' :
				...
				if($twobj28_state == 0){
					...
					$res = check_format_bj28($message_data['content'],$limit[2][0],C('bj28qi_max_point'),C('bj28_xz_open'),C('bj28_xz_max'));
					if($res['error']==0 || $res['error']==2){
						...
						if ($res['error'] == 0) {
							$content_msg =  '「'.$message_data['content'].'」'.'单笔点数最低'.$limit[2][0].',竞猜失败';
						} else {
